...
import { BiDotsVerticalRounded, BiWindowOpen } from 'react-icons/bi';
...
function LoginPage({ history }) {
  ...
  const { setLoggedIn, setSelectedChatId, setChatPartner, setUser, setMessages, setAllUsers } =
    ...
    try {
      ...
      if (email.current.value !== '' && password.current.value !== '') {
        const res = await getRequest('users/login', {
          headers: {
            Authorization: `Basic ${base64([email.current.value, password.current.value].join(':'))}`,
          },
        });
        if (res.status === 200) {
          setValidation(true);
  ...
  return (
    <div className='mainDiv'>
        ...
        <Row className='mt-5 ml-5 d-flex'>
          <Col sm={12} md={8} className='px-5'>
            ...
            <ol>
              ...
              <li className='LoginLi mt-3 mb-5'>Point your phone to this screen to capture the code</li>
              <a
          ...
          <Col className='mb-2' sm={12} md={4}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <QRCode className='img-fluid mx-auto' value='hey Buddy! Nothing to Link here atm :/' />
              <div
                style={{
                  display: 'flex',
                  flexDirection: 'row',
      ...
      <div
        ...
        <Container>
          <Form className='py-5 px-5 w-sm-100 w-lg-50 mx-auto' onSubmit={(e) => e.preventDefault()}>
            <Form.Group className='mb-3' controlId='formBasicEmail'>
              <Form.Label>Email</Form.Label>
              <Form.Control isInvalid={!validation} type='text' ref={email} placeholder='Enter your email' />
            </Form.Group>

            <Form.Group className='mb-3'>
              <Form.Label>Password</Form.Label>
              <Form.Control isInvalid={!validation} type='password' ref={password} placeholder='Password' />
            </Form.Group>
            {!validation && <p className='wrongValidation'>Wrong Credentials </p>}
            <div
              style={{
                display: 'flex',
                flexDirection: 'row',
              ...
              <div className='GreenLink ml-2 ml-md-0' onClick={() => setSignUp(true)}>
